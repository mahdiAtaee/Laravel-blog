...
<body dir="rtl" style="text-align: right;">
    <div class="main-wrapper">
        ...
        <div class="col-lg-12 col-12" style="margin-top: 11rem !important;">
            ...
            <h6 class="mb-15">ویرایش کاربر</h6>
            <form action="{{route('update.user',$userData->id)}}" method="post" enctype="multipart/form-data">
                @method('put')
                @csrf
                <div class="row mbn-15">
                    <div class="col-12 mb-3">
                        <input type="text" class="form-control" name="name" placeholder="نام" value="{{$userData->name}}">
                        @error('name')
                        <div class="alert alert-danger">{{$message}}</div>
                        @enderror
                    </div>
                    <div class="col-12 mb-3">
                        <input type="email" class="form-control" name="email" placeholder="ایمیل" value="{{$userData->email}}">
                        @error('email')
                        <div class="alert alert-danger">{{$message}}</div>
                        @enderror
                    </div>
                    <div class="col-12 mb-3">
                        <input type="password" class="form-control" name="password" placeholder="رمز عبور خود را وارد کنید" value="{{$userData->password}}">
                        @error('password')
                        <div class="alert alert-danger">{{$message}}</div>
                        @enderror
                    </div>
                    <div class="col-12 mb-3">
                        <textarea class="form-control" name="user_description" placeholder="توضیحات راجع به خود">{{$userData->user_description}}</textarea>
                    ...
                    <div class="col-12 mb-3">
                        <select class="form-control" name="rule">
                            ...
                            <option value="1" <?php if ($userData->rule == 1) {
                                                    echo "selected='selected'";
                                                } ?>>ادمین</option>
                            <option value="0" <?php if ($userData->rule == 0) {
                                                    echo "selected='selected'";
                                                } ?>>عادی</option>
                        </select>
                    </div>
